  ...
  return (
    <Layout>
      <div className="container" style={{ paddingBottom: '4rem', paddingTop: 'clamp(1rem, 2.5vw, 2rem)' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: '1 1 280px', minWidth: 0 }}>
            <h2 style={{
              ...
              <Layers size={24} style={{ flexShrink: 0 }} /> Semester Templates
            </h2>
        ...
        {successMsg && (
          <div className="alert success" style={{ marginBottom: '1.25rem', background: '#d1fae5', color: '#065f46', border: '1px solid #10b981', padding: '0.85rem 1rem', borderRadius: '6px', fontSize: '0.88rem', wordBreak: 'break-word' }}>
            ✓ {successMsg}
          </div>
        )}
        ...
        <div style={{
          ...
          <div style={{ flex: '1 1 120px', minWidth: '110px' }}>
            ...
            <select
              ...
              onChange={(e) => { setSelectedSemester(e.target.value); setHasLoaded(false); setTemplates([]); }}
              style={{ width: '100%', boxSizing: 'border-box', padding: '0.6rem 0.8rem', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '0.88rem', background: 'white', height: '38px' }}
            >
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', flex: '1 1 auto' }}>
            <button
              ...
              disabled={!selectedProgramId || !selectedSemester || loading}
              style={{
                flex: '1 1 130px',
        ...
        {hasLoaded && !loading && templates.length === 0 && (
          <div style={{
            background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: 'clamp(2rem, 5vw, 3rem) 1rem',
            textAlign: 'center', color: '#94a3b8'
          }}>
            <Layers size={40} style={{ marginBottom: '0.75rem', opacity: 0.4 }} />
            <p style={{ margin: 0, fontSize: '1rem', fontWeight: 600, color: '#475569' }}>No templates found for this program & semester.</p>
        ...
        {templates.map(template => (
          <div key={template.template_id} style={{
            background: '#ffffff',
            border: template.is_default ? '2px solid #f59e0b' : '1px solid #e2e8f0',
            borderRadius: '8px',
            marginBottom: '1rem',
            overflow: 'hidden',
            boxShadow: '0 1px 3px rgba(0,0,0,0.02)',
            ...
            <div style={{
              ...
              gap: '0.75rem',
              cursor: 'pointer',
              background: expandedId === template.template_id ? '#f8fafc' : 'transparent'
            }}
              onClick={() => setExpandedId(expandedId === template.template_id ? null : template.template_id)}
            >
              <div style={{ flex: '1 1 220px', minWidth: 0 }}>
                <div style={{ fontWeight: 700, color: '#002147', fontSize: '0.98rem', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.45rem', wordBreak: 'break-word' }}>
                  <span>{template.template_name}</span>
                  {template.is_default && (
                    <span style={{ fontSize: '0.65rem', background: '#fef3c7', color: '#b45309', border: '1px solid #fcd34d', padding: '0.1rem 0.4rem', borderRadius: '3px', fontWeight: 800, display: 'inline-flex', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', flexShrink: 0 }}>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', padding: '0.2rem' }}>
            ...
            {expandedId === template.template_id && (
              <div style={{ borderTop: '1px solid #e2e8f0', padding: 'clamp(0.75rem, 2vw, 1.25rem)' }}>
                {template.groups.map((group, gIdx) => (
                  <div key={group.group_id} style={{
                    background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px',
                    padding: 'clamp(0.6rem, 2vw, 0.85rem)', marginBottom: gIdx < template.groups.length - 1 ? '0.5rem' : 0
                  }}>
                    <div style={{ fontWeight: 700, color: '#334155', fontSize: '0.82rem', marginBottom: '0.45rem', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.35rem' }}>
                      {group.group_label
                      ...
                      {group.subjects.length > 1 && !group.group_label && (
                        <span style={{ color: '#94a3b8', fontWeight: 400 }}>(multiple subjects)</span>
                      )}
                      {group.subjects.length > 1 && group.group_label && (
                        <span style={{ color: '#94a3b8', fontWeight: 400 }}>(pick 1 of {group.subjects.length})</span>
                      )}
                    </div>
                    {group.subjects.map(sub => (
                      <div key={sub.subject_id} style={{
                        ...
                        padding: '0.4rem 0', fontSize: '0.82rem', color: '#334155', borderBottom: '1px solid #f1f5f9'
                      }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flex: '1 1 200px', minWidth: 0, wordBreak: 'break-word' }}>
                          <span style={{ fontWeight: 700, color: '#002147', flexShrink: 0 }}>{sub.subject_code}</span>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', flexWrap: 'wrap', flexShrink: 0 }}>
                          {sub.branch && (
                            <span style={{ background: 'rgba(0,33,71,0.06)', color: '#002147', padding: '0.1rem 0.35rem', borderRadius: '3px', fontSize: '0.72rem', fontWeight: 600 }}>
                              {sub.branch}
                            </span>
                          )}
                          {sub.scheme_detail && (
                            <span style={{ background: '#ffffff', color: '#64748b', padding: '0.1rem 0.35rem', borderRadius: '3px', fontSize: '0.72rem', border: '1px solid #e2e8f0' }}>
                              {sub.scheme_detail}
                            </span>
                          )}
                          <span style={{ color: '#64748b', fontSize: '0.72rem', fontWeight: 600, background: '#e2e8f0', padding: '0.1rem 0.35rem', borderRadius: '3px' }}>
                            {sub.credit} cr
                          </span>
                        </div>
                      </div>
      ...
      {showModal && (
        <div style={{
          ...
          zIndex: 1000, padding: 'clamp(0.5rem, 2vw, 1.5rem)', overflowY: 'auto'
        }}>
          <div style={{
            background: '#ffffff', borderRadius: '8px', maxWidth: '750px', width: '100%',
            maxHeight: '92vh', display: 'flex', flexDirection: 'column',
            boxShadow: '0 20px 25px -5px rgba(0,0,0,0.15)', overflow: 'hidden'
          }}>
            {/* Modal Header */}
            <div style={{
              background: '#002147', color: '#ffffff', padding: 'clamp(0.85rem, 2vw, 1.2rem) clamp(1rem, 2.5vw, 1.35rem)',
              ...
              <button onClick={closeModal}
                style={{ background: 'none', border: 'none', color: '#ffffff', fontSize: '1.5rem', cursor: 'pointer', lineHeight: 1, padding: '0 0.25rem' }}>
                ×
              </button>
            </div>

            {/* Modal Body */}
            <div style={{ padding: 'clamp(0.85rem, 2.5vw, 1.5rem)', overflowY: 'auto', flex: 1 }}>
              {modalError && (
                <div className="alert error" style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.88rem', wordBreak: 'break-word' }}>
              ...
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                <input
                  ...
                  onChange={(e) => setModalForm(prev => ({ ...prev, template_name: e.target.value }))}
                  placeholder='e.g. "Regular Track", "Honors / Minor Track"'
                  style={{ width: '100%', boxSizing: 'border-box', padding: '0.6rem 0.8rem', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '0.9rem' }}
                />
              </div>
              ...
              <div style={{ marginBottom: '0.6rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
                ...
                <button type="button" onClick={addGroup} style={{
                  ...
                  padding: '0.35rem 0.75rem', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700,
              ...
              {modalForm.groups.map((group, idx) => (
                <div key={idx} style={{
                  border: '1px solid #e2e8f0', borderRadius: '6px', padding: 'clamp(0.7rem, 2vw, 1rem)', marginBottom: '0.85rem',
                  background: '#fafbfc'
                }}>
                  ...
                  <div style={{ marginBottom: '0.75rem' }}>
                    ...
                    <input
                      ...
                      style={{ width: '100%', boxSizing: 'border-box', padding: '0.5rem 0.7rem', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '0.82rem' }}
                    />
                  </div>
            ...
            <div style={{
              ...
              <button
                ...
                onClick={closeModal}
                className="btn btn-outline"
                style={{ borderColor: '#cbd5e1', color: '#64748b', fontWeight: 600, borderRadius: '4px', padding: '0.55rem 1.1rem', fontSize: '0.85rem' }}
              >
                Cancel
              </button>
              <button
      ...
      {deleteConfirm && (
        <div style={{
          position: 'fixed', top: 0, left: 0, right: 0, bottom: 0,
          background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center',
          zIndex: 1001, padding: 'clamp(0.5rem, 2vw, 1rem)'
        }}>
          <div style={{
            ...
            <div style={{ padding: 'clamp(1rem, 3vw, 1.5rem)' }}>
              <h3 style={{ margin: '0 0 0.5rem 0', color: '#dc2626', fontSize: '1.05rem', fontWeight: 700 }}>
                Delete Template?
              </h3>
            ...
            <div style={{
              padding: '0.75rem clamp(1rem, 3vw, 1.5rem)', background: '#f8fafc', borderTop: '1px solid #e2e8f0',
              display: 'flex', justifyContent: 'flex-end', gap: '0.6rem', flexWrap: 'wrap'
            }}>
              <button type="button" onClick={() => setDeleteConfirm(null)}
                style={{ padding: '0.5rem 1rem', background: 'white', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer', fontWeight: 600, color: '#64748b', fontSize: '0.85rem' }}>
                Cancel
              </button>
              <button type="button" onClick={() => handleDelete(deleteConfirm)} disabled={deleteLoading}
                style={{
                  ...
                  borderRadius: '4px', cursor: 'pointer', fontWeight: 700, fontSize: '0.85rem', opacity: deleteLoading ? 0.6 : 1
                }}>
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', marginBottom: '0.5rem', minHeight: '28px' }}>
        {selectedIds.map(id => {
          ...
          return (
            <span key={id} style={{
              ...
              padding: '0.25rem 0.5rem', fontSize: '0.76rem', fontWeight: 600,
              display: 'inline-flex', alignItems: 'center', gap: '0.35rem', maxWidth: '100%', wordBreak: 'break-word'
            }}>
              <span>{sub ? `${sub.subject_code} – ${sub.subject_name}` : `Subject #${id}`}</span>
              {sub?.branch && (
                <span style={{ background: '#e0e7ff', color: '#3730a3', fontSize: '0.68rem', padding: '0.1rem 0.3rem', borderRadius: '3px', flexShrink: 0 }}>
                  {sub.branch}
                </span>
              )}
              <button
                type="button"
                onClick={() => onToggleSubject(groupIdx, id)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1e40af', fontSize: '1rem', padding: '0 0 0 2px', lineHeight: 1, display: 'flex', alignItems: 'center', flexShrink: 0 }}
              >
                ×
              </button>
            </span>
        ...
        {selectedIds.length === 0 && <span style={{ color: '#94a3b8', fontSize: '0.8rem' }}>No subjects selected for this group</span>}
      </div>
      ...
      <button
        ...
        style={{
          ...
          background: open ? '#f8fafc' : 'white', cursor: 'pointer', textAlign: 'left', fontSize: '0.82rem', color: '#1e293b',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 500, boxSizing: 'border-box'
        }}
      >
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginRight: '0.5rem' }}>
          {open ? '▲ Close subject browser' : `▼ Browse & select subjects (${availableSubjects.length} available)`}
        </span>
      ...
      {open && (
        <div style={{
          ...
          background: 'white', maxHeight: '360px', overflowY: 'auto', boxShadow: '0 8px 16px rgba(0,0,0,0.1)'
        }}>
          {/* Search & Quick Filters Header */}
          <div style={{ padding: '0.6rem', borderBottom: '1px solid #e2e8f0', position: 'sticky', top: 0, background: '#f8fafc', zIndex: 2 }}>
            <div style={{ position: 'relative', marginBottom: '0.45rem' }}>
              ...
              <input
                type="text"
                placeholder="Search by code (e.g. OEC-), title..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                style={{ width: '100%', boxSizing: 'border-box', padding: '0.45rem 0.6rem 0.45rem 2rem', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '0.82rem', background: 'white' }}
              />
              {searchTerm && (
                <button
                  type="button"
            ...
            <div style={{ display: 'flex', gap: '0.45rem', flexWrap: 'wrap', alignItems: 'center', fontSize: '0.78rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flex: '1 1 110px' }}>
                <span style={{ color: '#64748b', fontWeight: 600, fontSize: '0.72rem' }}>Scheme:</span>
                <select
                  value={filterScheme}
                  onChange={(e) => setFilterScheme(e.target.value)}
                  style={{ width: '100%', padding: '0.25rem 0.35rem', border: '1px solid #cbd5e1', borderRadius: '4px', fontSize: '0.72rem', background: 'white' }}
                >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', flex: '1 1 120px' }}>
                <span style={{ color: '#64748b', fontWeight: 600, fontSize: '0.72rem' }}>Dept:</span>
                <select
                  value={filterDept}
              ...
              <span style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 600, width: '100%', textAlign: 'right' }}>
            ...
            {filtered.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.45rem', paddingTop: '0.4rem', borderTop: '1px solid #e2e8f0' }}>
                <button
                  type="button"
                  onClick={() => {
          ...
          {subjectsLoading ? (
            ...
            filtered.map(s => {
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', flex: '1 1 180px', minWidth: 0 }}>
                    <div style={{
                      width: '16px',
                      height: '16px',
                      borderRadius: '3px',
                    ...
                    <div style={{ wordBreak: 'break-word' }}>
                      ...
                      <span style={{ color: '#334155', marginLeft: '0.35rem' }}>{s.subject_name}</span>
                    </div>
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', flexWrap: 'wrap', flexShrink: 0, alignSelf: 'center' }}>
                    {s.branch && (
                      <span style={{ background: 'rgba(0,33,71,0.06)', color: '#002147', padding: '0.1rem 0.35rem', borderRadius: '3px', fontSize: '0.7rem', fontWeight: 600, maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={s.branch}>
                        {s.branch}
                      </span>
                    )}
                    {s.scheme_detail && (
                      <span style={{ background: '#f8fafc', color: '#64748b', padding: '0.1rem 0.35rem', borderRadius: '3px', fontSize: '0.7rem', border: '1px solid #e2e8f0' }}>
                        {s.scheme_detail}
                      </span>
                    )}
                    <span style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 600, background: '#f1f5f9', padding: '0.1rem 0.3rem', borderRadius: '3px' }}>
                      {s.credit} cr
                    </span>
                  </div>
                </div>
